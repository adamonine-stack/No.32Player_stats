...
test('Kyoto historical import menu is wired into settings',()=>{
  assert.match(app,/openKyotoHistoryImportMenu\(\).*京都府 2025・2026大会を登録/);
  assert.match(app,/const KYOTO_HISTORY_IMPORTS=\[/);
  assert.match(app,/TOURNAMENT_2026_KYOTO_U15_MEN/);
  assert.match(app,/TOURNAMENT_2026_KYOTO_JHS_SOUTAI_MEN/);
...
test('Kyoto imports preserve explicit similar-name decisions and qualifier separation',()=>{
  assert.match(app,/類似チーム名がある場合は、次の画面で統合するか別チームとして登録するか選択できます/);
  assert.match(app,/予選として登録する大会は履歴には残しますが、ランク・Team Powerには加算しません/);
  assert.match(app,/data-kyoto-import/);
  assert.match(app,/teamPowerEligible:tournament\.teamPowerEligible!==false&&imported\.teamPowerEligible!==false/);
  assert.doesNotMatch(app,/TOURNAMENT_2025_KYOTO_JR_WINTER_TERM1_MEN/);
});
