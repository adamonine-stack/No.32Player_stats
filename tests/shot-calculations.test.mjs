...
assert.equal(detectShotArea(50, 19.167), "under_basket");
assert.equal(detectShotArea(50, 19.2), "inside");
assert.equal(detectShotArea(50, 24), "inside");
assert.equal(detectShotArea(41.333, 10.5), "under_basket");
assert.equal(detectShotArea(41.3, 10.5), "inside");
assert.equal(detectShotArea(36, 10.5), "inside");
...
const staleUnderBasketShot = { ...underMade, shotArea: "under_basket", shotAreaLabel: "ゴール下", shotX: 50, shotY: 24 };
assert.equal(shotAreaForRecord(staleUnderBasketShot), "inside", "stored shots are reclassified from coordinates using the latest boundary");
assert.equal(normalizeShot(staleUnderBasketShot).shotAreaLabel, "インサイド");
assert.deepEqual(aggregateShots([staleUnderBasketShot], "shotArea", SHOT_AREA_ORDER).inside, { made: 1, attempts: 1, registered: 1 });
