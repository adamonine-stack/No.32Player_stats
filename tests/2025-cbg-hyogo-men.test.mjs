...
assert.equal(rank('ゴッドドア'),'S');assert.equal(rank('KARTER'),'A+');assert.equal(rank('HDC Academy Cranes'),'A');assert.equal(rank('G.Dark Horse'),'A');assert.equal(rank('EPIC BASKETBALL CLUB U15'),'B');assert.equal(rank('TEDDY BASKETBALL CLUB'),'B');assert.equal(rank('センターサークル'),'B');assert.equal(rank('ARMS'),'B');assert.equal(rank('BRUINS ashiya'),'C');assert.equal(rank('神戸ストークスU14'),'D');assert.equal(rank('All Blacks A'),'E');
