import {createHistoryOverlay,applyHistoryPatch} from '../js/core/history-journal.js';
...
test('①②⑬⑭ actual participation save writes games participation only, all Q data unchanged',async()=>{
 ...
 const writes=[];const c=vm.createContext({Object,structuredClone,createHistoryOverlay,state:{games:[h.docs.get('games/g')]},db:{},doc:(_,collection,id)=>({collection,id}),serverTimestamp:()=>123,submitOfflineCapable:async(type,payload,save,options)=>{assert.equal(type,'gamePatch');assert.equal(save,null);assert.equal(options.overlay.documents.length,1);const change=options.overlay.documents[0];writes.push(change);Object.assign(h.docs.get('games/g'),applyHistoryPatch(h.docs.get('games/g'),change.patch));return {queued:true}},setDoc:async(ref,data)=>{writes.push({ref,data});assert.equal(ref.collection,'games');assert.equal('playEvents' in data,false);Object.assign(h.docs.get('games/g'),structuredClone(data))}});vm.runInContext(source,c);
...
test('normal absolute save verifies baseline before staging and keeps guarded transaction',()=>{const start=app.indexOf('async function saveAssistPlay('),end=app.indexOf('\nfunction assistPlayerLabel',start),body=app.slice(start,end),guard=body.slice(0,body.indexOf('  game=state.games'));assert.match(guard,/assertNormalStatsOnline/);assert.match(guard,/if\(!action.baseline\)throw Error\(LOAD_ERROR\)/);assert.match(guard,/submitOfflineCapable/);assert.match(body.slice(guard.length),/submitOfflineCapable\('assist'/)});
