...
for(const name of ['assist-play-store.js','quick-history-store.js']){const p=path.join(target,'js/core',name);fs.writeFileSync(p,fs.readFileSync(p,'utf8').replace("'https://www.gstatic.com/firebasejs/12.15.0/firebase-firestore.js'","'./firebase.js?v=20260901-scoped-reads-v1'"))}
...
const p=path.join(target,'js/app.js');fs.writeFileSync(p,fs.readFileSync(p,'utf8').replace("if('serviceWorker' in navigator)","if(false)"));
console.log(target);
...
const queue=fs.readFileSync(path.join(root,'js/core/offline-operation-queue.js'),'utf8').replace('r32-offline-operations','r32-qa-offline-operations');
