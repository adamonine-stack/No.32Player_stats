...
test('same-event concurrent edits sync and the later confirmed edit wins the same field',async()=>{const c=client();c.assist({kind:'saveShot',shot});const s=server();await run(s,c.ops);const a=client('C',s.game(),s.stats()),b=client('B',s.game(),s.stats());a.assist({kind:'saveShot',shot:{...shot,shotX:51},edit:true});b.assist({kind:'saveShot',shot:{...shot,shotX:52},edit:true});await run(s,a.ops);await run(s,b.ops);assert.equal(s.stats().find(x=>x.playerId==='p').quarters.q1.shots[0].shotX,52);assert.equal(s.game().playEvents.length,1)});
test('same-event concurrent edits rebase without dropping unrelated remote fields',async()=>{const c=client();c.assist({kind:'saveShot',shot});const s=server();await run(s,c.ops);const a=client('C',s.game(),s.stats()),b=client('B',s.game(),s.stats());a.assist({kind:'saveShot',shot:{...shot,shotX:51},edit:true});b.assist({kind:'saveShot',shot:{...shot,result:'missed'},edit:true});await run(s,a.ops);await run(s,b.ops);const finalShot=s.stats().find(x=>x.playerId==='p').quarters.q1.shots[0];assert.equal(finalShot.shotX,51);assert.equal(finalShot.result,'missed')});
