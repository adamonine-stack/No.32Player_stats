...
assert.deepEqual(Object.fromEntries(['S','A+','A','B','C','D'].map(rank=>[rank,TEAMS_2026_KYOTO_U15_MEN.filter(team=>team.rank===rank).length])),{S:1,'A+':1,A:2,B:4,C:8,D:0});
...
assert.equal(calculateOpponentTeamRank([{placementRank:'A',tournamentLevel:'prefecture'},{placementRank:'B',tournamentLevel:'prefecture'}]).rank,'A');
