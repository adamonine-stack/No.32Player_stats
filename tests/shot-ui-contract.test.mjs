...
assert.match(app, /const selectedAreas=new Set\(\)/);
assert.match(app, /selectedAreas\.has\(shotAreaForRecord\(shot\)\)/);
assert.match(app, /if\(selectedAreas\.has\(areaId\)\)selectedAreas\.delete\(areaId\);else selectedAreas\.add\(areaId\)/);
assert.match(app, /distanceFilter='all';sideFilter='all';selectedType=''/);
assert.match(app, /selectedAreas\.clear\(\);selectedType='';if\(field==='distance'\)/);
assert.match(app, /shotCourtSvg\(displayShots,null,'',\[\.\.\.selectedAreas\]\)/);
