...
function renderHome(){
  ...
  views.home.innerHTML = `<div class="card hero"><span class="eyebrow">Dashboard</span><h2>${p ? `No.${p.number||'-'} ${p.name}` : '選手を登録してください'}</h2><p class="muted">${p?`${p.position||''} ${p.height?`｜${p.height}cm`:''}`:'選手タブから登録できます。'}</p><button class="primary-btn" id="goInput">＋ 試合を登録</button></div>
  ${p?statCards(avg,'1試合平均スタッツ'):''}
...
function renderStats(){
  ...
  <div class="card"><span class="eyebrow">対象</span><h2>${title}</h2><p class="muted">総Q数 ${one(total.q)}Q ｜ 4Q換算 ${one(total.q/4)}試合</p>${state.statMode!=='year'?matchList(target):''}</div>
  ${statCards(avg,'4Q換算 1試合平均')}${breakdowns(avg)}${state.statMode==='game'?videoButtons(target[0]):''}`;
