...
function miniCards(a){
  return `<div class="summary-grid">
    ${card('総Q', a.totalQ, '対象選手')}${card('4Q換算', fmt(a.equivGames,2), '試合数')}${card('PTS', fmt(a.perGame.pts), '1試合換算')}${card('FG%', fmt(a.fgPct)+'%', `${a.fgm}/${a.fga}`)}
  </div>`;
}
...
function renderGames(){
  $('#view-games').innerHTML = `<div class="section-head"><div><h2>試合</h2><p>登録済み試合の結果一覧。修正ボタンから試合情報とスタッツを編集できます。</p></div><button class="primary-btn admin-only" id="gamesNewBtn">＋ ホームと同じ入力画面を開く</button></div>
