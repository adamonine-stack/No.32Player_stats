...
function renderPlayers(){
  ...
  views.players.innerHTML = `<div class="card"><div class="section-title">${state.editingPlayerId?'選手修正':'選手登録'}</div><div class="form-grid"><input id="pNo" placeholder="背番号" value="${editing.number||''}"><input id="pName" placeholder="選手名" value="${editing.name||''}"><input id="pPos" placeholder="ポジション" value="${editing.position||''}"><input id="pHeight" placeholder="身長(cm)" type="number" value="${editing.height||''}">${state.editingPlayerId?'<button id="cancelPlayerEdit" class="ghost-btn">キャンセル</button>':''}<button id="addPlayer" class="primary-btn ${state.editingPlayerId?'':'full'}">${state.editingPlayerId?'更新':'選手を追加'}</button></div></div><div class="card"><div class="section-title">選手一覧</div>${state.players.map(p=>`<div class="list-item player-row"><div><b>No.${p.number||'-'} ${p.name}</b><div class="muted">${p.position||''} ${p.height?`｜${p.height}cm`:''}</div></div><div class="row-actions"><button class="ghost-btn select-player" data-id="${p.id}">選択</button><button class="ghost-btn edit-player" data-id="${p.id}">修正</button><button class="danger-btn delete-player" data-id="${p.id}">削除</button></div></div>`).join('')||'<p class="muted">まだ選手がありません</p>'}</div>`;
