...
function lastPlayerId(){ return state.analysis.playerId || localStorage.getItem(state.lastPlayerKey) || state.data.players[0]?.id || ''; }
function setLastPlayer(id){ if(id){ state.analysis.playerId = id; localStorage.setItem(state.lastPlayerKey, id); } }
...
function init(){
  ...
  state.analysis.playerId = localStorage.getItem(state.lastPlayerKey) || '';
...
function setupFirebase(){
  ...
  ['players','games','stats','videos','settings'].forEach(name => {
    Firebase.listenCollection(name, rows => {
      ...
      console.error(err); state.cloud = false; setStatus('クラウド未接続｜ローカル表示中', false); render();
    });
  });
...
function setupNav(){
  $$('.nav-item').forEach(btn => btn.addEventListener('click', () => {
    ...
    if(state.activeView === 'games') state.edit = null;
...
async function saveGameAndStat(game, stat){
  await save('games', game);
  if(stat && stat.playerId) await save('stats', stat);
}
...
function selectStats(filter={}){
  ...
  const rangeIds = new Set(games.map(g=>g.id));
  let stats = state.data.stats.filter(s=>rangeIds.has(s.gameId));
  if(filter.playerId){
    stats = stats.filter(s=>s.playerId===filter.playerId);
    const playedIds = new Set(stats.map(s=>s.gameId));
    games = games.filter(g=>playedIds.has(g.id));
  }
  return {games, stats};
}
...
function statFor(gameId, playerId){ return state.data.stats.find(s=>s.gameId===gameId && s.playerId===playerId); }

function render(){
  if(!document.body.classList.contains('is-admin')) document.body.classList.add('not-admin');
  if(!state.analysis.playerId && state.data.players[0]) setLastPlayer(state.data.players[0].id);
  renderHome(); renderPlayers(); renderGames(); renderAnalysis(); renderVideos(); renderSettings();
}
function miniCards(a){
  const p = a.perGame;
  return `<div class="summary-grid">
    ${card('2P%', fmt(p.fg2Pct)+'%', `${fmt(p.fg2m)}/${fmt(p.fg2a)}`)}${card('3P%', fmt(p.fg3Pct)+'%', `${fmt(p.fg3m)}/${fmt(p.fg3a)}`)}${card('FG%', fmt(p.fgPct)+'%', `${fmt(p.fgm)}/${fmt(p.fga)}`)}${card('TO', fmt(p.to), '1試合平均')}
  </div>`;
}
...
function renderHome(){
  ...
  const a = aggregate({playerId:pid});
  const recent = a.games.slice(-5).reverse();
  $('#view-home').innerHTML = `<div class="home-dashboard">
    <div class="hero-card compact-hero"><div class="hero-content"><div class="eyebrow">R32 Dashboard</div><h1>${player ? `#${escapeHtml(player.number||'')} ${escapeHtml(player.name||'')}` : '選手を選択してください'}</h1><p>${player ? `${escapeHtml(player.position||'')}｜${escapeHtml(player.grade||'')}｜${escapeHtml(player.height||'')}` : '選手登録後、スタッツ画面で選手を選択するとホームに表示されます。'}</p><div class="quick-actions"><button class="primary-btn admin-only" id="newGameBtn">＋ 試合入力</button><button class="secondary-btn" data-go="analysis">スタッツを見る</button></div></div></div>
    <div>${miniCards(a)}</div>
  </div>
  <div class="section-head"><div><h2>1試合平均スタッツ</h2><p>最後に選択した選手のすべてのデータを4Q換算した1試合平均</p></div></div>
  ${homeStatsGrid(a)}
  <div class="section-head"><div><h2>最近の対象試合</h2><p>${player ? escapeHtml(player.name||'') : '選択選手'} の直近データ</p></div></div>
  ...
  const nb = $('#newGameBtn'); if(nb) nb.onclick=()=>openGameForm(null, pid);
}
function homeStatsGrid(a){ const p=a.perGame; return `<div class="analysis-card stat-matrix home-stat-matrix">
  ${metricCell('2PA / 2PM / 2P%', `${fmt(p.fg2a)} / ${fmt(p.fg2m)} / <strong>${fmt(p.fg2Pct)}%</strong>`, '試合換算')}
  ${metricCell('3PA / 3PM / 3P%', `${fmt(p.fg3a)} / ${fmt(p.fg3m)} / <strong>${fmt(p.fg3Pct)}%</strong>`, '試合換算')}
  ${metricCell('FGA / FGM / FG%', `${fmt(p.fga)} / ${fmt(p.fgm)} / <strong>${fmt(p.fgPct)}%</strong>`, '試合換算')}
  ${metricCell('FTA / FTM / FT%', `${fmt(p.fta)} / ${fmt(p.ftm)} / <strong>${fmt(p.ftPct)}%</strong>`, '試合換算')}
  ${metricCell('AST', fmt(p.ast), '試合換算')}
  ${metricCell('REB', fmt(p.reb), '試合換算')}
  ${metricCell('STL', fmt(p.stl), '試合換算')}
  ${metricCell('BLK', fmt(p.blk), '試合換算')}
  ${metricCell('TO', fmt(p.to), '試合換算')}
</div>`; }
...
function openGameForm(gameId=null, playerId=lastPlayerId()){ state.activeView='games'; state.edit={gameId, playerId}; go('games'); }
...
function gameItem(g){ const stats = state.data.stats.filter(s=>s.gameId===g.id); const pts = stats.reduce((a,s)=>a+num(s.fg2m)*2+num(s.fg3m)*3+num(s.ftm),0); return `<div class="item"><div><strong>${escapeHtml(g.opponent||'対戦相手未入力')}｜${scoreText(g)}</strong><small>${escapeHtml(g.date||'')}｜${escapeHtml(g.event||'')}｜${num(g.quarters||4)}Q｜PTS ${pts}</small></div><div class="chip">${escapeHtml(gameResult(g))}</div></div>`; }
...
function renderPlayers(){
  ...
  $$('.select-player').forEach(b=>b.onclick=()=>{ setLastPlayer(b.dataset.id); state.analysis.playerId=b.dataset.id; go('analysis'); });
  $$('.delete-player').forEach(b=>b.onclick=()=>remove('players', b.dataset.id));
}
function playerItem(p){ return `<div class="item"><div><strong>#${escapeHtml(p.number||'')} ${escapeHtml(p.name||'')}</strong><small>${escapeHtml(p.position||'')}｜${escapeHtml(p.grade||'')}｜${escapeHtml(p.height||'')}</small></div><div class="item-actions"><button class="icon-btn select-player" data-id="${p.id}">選択</button><button class="icon-btn admin-only delete-player" data-id="${p.id}">削除</button></div></div>`; }

function renderGames(){
  $('#view-games').innerHTML = `<div class="section-head"><div><h2>試合</h2><p>登録済み試合の結果一覧。修正ボタンから試合情報とスタッツを編集できます。新規登録はホームの「＋ 試合入力」から行います。</p></div></div>
  <p class="notice lock-note">閲覧モードです。修正には管理者ログインが必要です。</p>
  ${state.edit ? gameInputPanel() : ''}
  <div class="list-card"><h3>登録済み試合一覧</h3><div class="list">${state.data.games.length ? [...state.data.games].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(gameItemWithActions).join('') : empty()}</div></div>`;
  const newBtn=$('#gamesNewBtn'); if(newBtn) newBtn.onclick=()=>{state.edit={gameId:null, playerId:lastPlayerId()}; renderGames();};
  setupGameFormHandlers();
  $$('.edit-game').forEach(b=>b.onclick=()=>{ state.edit={gameId:b.dataset.id, playerId:lastPlayerId()}; renderGames(); });
  $$('.delete-game').forEach(b=>b.onclick=()=>remove('games', b.dataset.id));
}
function gameInputPanel(){
  const game = state.edit?.gameId ? gameById(state.edit.gameId) : null;
  const pid = state.edit?.playerId || lastPlayerId();
  const stat = game ? statFor(game.id, pid) : null;
  const playerOptions = state.data.players.map(p=>`<option value="${p.id}" ${p.id===pid?'selected':''}>#${escapeHtml(p.number||'')} ${escapeHtml(p.name||'')}</option>`).join('');
  const val = (obj,k,d='') => obj?.[k] ?? d;
  return `<form class="form-card admin-only" id="combinedGameForm"><div class="form-title-row"><h3>${game ? '試合・スタッツ修正' : '新規試合入力'}</h3><button type="button" class="icon-btn" id="closeGameForm">閉じる</button></div>
  <input type="hidden" name="gameId" value="${escapeHtml(game?.id||'')}"><input type="hidden" name="statId" value="${escapeHtml(stat?.id||'')}">
  <h4>試合情報</h4><div class="field-3"><label>年月日<input name="date" type="date" value="${escapeHtml(val(game,'date',toDateInput()))}" required></label><label>対戦校<input name="opponent" value="${escapeHtml(val(game,'opponent',''))}" placeholder="例 サンダース中学校"></label><label>大会名<input name="event" value="${escapeHtml(val(game,'event',''))}" placeholder="例 U15春季大会"></label></div>
  <div class="field-3"><label>会場<input name="place" value="${escapeHtml(val(game,'place',''))}"></label><label>Q数<input name="quarters" type="number" min="1" max="12" value="${escapeHtml(val(game,'quarters',4))}"></label><label>自チーム得点<input name="scoreFor" type="number" min="0" value="${escapeHtml(val(game,'scoreFor',''))}"></label><label>相手得点<input name="scoreAgainst" type="number" min="0" value="${escapeHtml(val(game,'scoreAgainst',''))}"></label><label>結果<select name="result"><option value="">スコアから自動</option><option ${gameResult(game||{})==='勝ち'?'selected':''}>勝ち</option><option ${gameResult(game||{})==='負け'?'selected':''}>負け</option><option ${gameResult(game||{})==='引分'?'selected':''}>引分</option></select></label></div>
  <h4>対象選手</h4><label>選手<select name="playerId" id="editPlayerSelect" required>${playerOptions}</select></label>
  <h4>シュート</h4><div class="field-3"><label>2PA（2P試投）<input name="fg2a" type="number" min="0" value="${val(stat,'fg2a',0)}"></label><label>2PM（2P成功）<input name="fg2m" type="number" min="0" value="${val(stat,'fg2m',0)}"></label><label>3PA（3P試投）<input name="fg3a" type="number" min="0" value="${val(stat,'fg3a',0)}"></label><label>3PM（3P成功）<input name="fg3m" type="number" min="0" value="${val(stat,'fg3m',0)}"></label><label>FTA（FT試投）<input name="fta" type="number" min="0" value="${val(stat,'fta',0)}"></label><label>FTM（FT成功）<input name="ftm" type="number" min="0" value="${val(stat,'ftm',0)}"></label></div>
  <h4>基本スタッツ</h4><div class="field-3"><label>AST（アシスト）<input name="ast" type="number" min="0" value="${val(stat,'ast',0)}"></label><label>OR（オフェンスリバウンド）<input name="orb" type="number" min="0" value="${val(stat,'orb',0)}"></label><label>DR（ディフェンスリバウンド）<input name="drb" type="number" min="0" value="${val(stat,'drb',0)}"></label></div>
  <h4>ボール奪取・ブロック</h4><div class="field-3"><label>PK（パスカット）<input name="stlPassCut" type="number" min="0" value="${val(stat,'stlPassCut',0)}"></label><label>DS（ドリブルカット）<input name="stlDribble" type="number" min="0" value="${val(stat,'stlDribble',0)}"></label><label>BLK（ブロック）<input name="stlBlock" type="number" min="0" value="${val(stat,'stlBlock',0)}"></label></div>
  <h4>ターンオーバー分類</h4><div class="field-3"><label>PM（パスミス）<input name="toPass" type="number" min="0" value="${val(stat,'toPass',0)}"></label><label>DM（ドリブルミス）<input name="toDribble" type="number" min="0" value="${val(stat,'toDribble',0)}"></label><label>CM（キャッチミス）<input name="toCatch" type="number" min="0" value="${val(stat,'toCatch',0)}"></label><label>VIO（バイオレーション）<input name="toViolation" type="number" min="0" value="${val(stat,'toViolation',0)}"></label><label>OTH（その他）<input name="toOther" type="number" min="0" value="${val(stat,'toOther',0)}"></label></div>
  <label>メモ<textarea name="note" placeholder="良かったプレー、課題、動画時間など">${escapeHtml(val(stat,'note',''))}</textarea></label><button class="primary-btn">保存</button></form>`;
}
function setupGameFormHandlers(){
  const form=$('#combinedGameForm');
  if(!form) return;
  $('#closeGameForm').onclick=()=>{ state.edit=null; renderGames(); };
  const playerSelect=$('#editPlayerSelect');
  if(playerSelect) playerSelect.onchange=()=>{ state.edit.playerId=playerSelect.value; setLastPlayer(playerSelect.value); renderGames(); };
  form.onsubmit=async e=>{
    e.preventDefault();
    const fd=Object.fromEntries(new FormData(form));
    const gameId = fd.gameId || uid();
    const game={id:gameId, date:fd.date, opponent:fd.opponent, event:fd.event, place:fd.place, quarters:num(fd.quarters||4), scoreFor:fd.scoreFor===''?'':num(fd.scoreFor), scoreAgainst:fd.scoreAgainst===''?'':num(fd.scoreAgainst), result:fd.result};
    if(!game.result && game.scoreFor !== '' && game.scoreAgainst !== '') game.result = game.scoreFor > game.scoreAgainst ? '勝ち' : game.scoreFor < game.scoreAgainst ? '負け' : '引分';
    const statKeys=['fg2a','fg2m','fg3a','fg3m','fta','ftm','ast','orb','drb','stlPassCut','stlDribble','stlBlock','toPass','toDribble','toCatch','toViolation','toOther'];
    const stat={id:fd.statId || uid(), gameId, playerId:fd.playerId, note:fd.note || ''};
    statKeys.forEach(k=>stat[k]=num(fd[k]));
    setLastPlayer(fd.playerId);
    await saveGameAndStat(game, stat);
    state.edit=null; render();
  };
}
function gameItemWithActions(g){ const pid=lastPlayerId(); const s=statFor(g.id,pid); const pts=s ? num(s.fg2m)*2+num(s.fg3m)*3+num(s.ftm) : 0; return `<div class="item"><div><strong>${escapeHtml(g.date||'')} VS ${escapeHtml(g.opponent||'')}｜${scoreText(g)}</strong><small>${escapeHtml(g.event||'')}｜${escapeHtml(g.place||'')}｜${num(g.quarters||4)}Q｜${escapeHtml(gameResult(g))}｜選択選手PTS ${pts}</small></div><div class="item-actions admin-only"><button class="icon-btn edit-game" data-id="${g.id}">修正</button><button class="icon-btn delete-game" data-id="${g.id}">削除</button></div></div>`; }

function renderAnalysis(){
  const t = state.analysis.type || 'game';
  const playerOptions = state.data.players.map(p=>`<option value="${p.id}" ${p.id===lastPlayerId()?'selected':''}>#${escapeHtml(p.number||'')} ${escapeHtml(p.name||'')}</option>`).join('');
  const opts = analysisOptions(t);
  if(!state.analysis.key && opts.length) state.analysis.key = opts[0].value;
  const pid = lastPlayerId();
  const a = aggregate({type:t, key:state.analysis.key, playerId:pid});
  ...
  $('#view-analysis').innerHTML = `<div class="analysis-mobile">
    <div class="analysis-top"><div><h2>📊 スタッツ <span>STATS</span></h2></div><button class="pill-btn ghost" id="filterReset">リセット</button></div>
    <div class="analysis-card range-card"><div class="card-title">対象選手とデータ範囲を選択</div>
    <label class="select-label">選手<select id="analysisPlayer">${playerOptions}</select></label>
    ...
    <div class="analysis-card player-focus"><span>選択選手</span><b>${player ? `#${escapeHtml(player.number||'')} ${escapeHtml(player.name||'')}` : '選手未選択'}</b><small>この選手のスタッツがホーム画面にも表示されます。</small></div>
    ${analysisHeader(t,title,a,game)}
    ${listNeeded ? matchListCard(a.games) : ''}
    ${overviewCard(a)}
    ${statsGrid(a)}
    <div class="breakdown-grid">${stlBreakdown(a)}${toBreakdown(a)}</div>
    <div class="analysis-card graph-box"><h3>📈 グラフで確認</h3><div class="graph-buttons"><button class="active">得点</button><button>シュート%</button><button>リバウンド</button><button>アシスト</button><button>ターンオーバー</button></div>${barChart(chartRowsFor(t, a.games, pid))}</div>
  </div>`;
  ...
  const ps=$('#analysisPlayer'); if(ps) ps.onchange=()=>{ setLastPlayer(ps.value); render(); };
  $('#filterReset').onclick=()=>{ state.analysis={type:'game', key:'', playerId:lastPlayerId()}; renderAnalysis(); };
}
...
function selectedTitle(t,key,games){
  if(t==='game'){ const g=games[0] || gameById(key); return g ? `${g.opponent||'対戦校未入力'} 戦` : '試合を選択'; }
...
function analysisHeader(t,title,a,game){
  if(t==='game'){ const base = game || gameById(state.analysis.key); if(base){ return `<div class="analysis-card selected-card"><div><div class="small-title">試合名</div><h2>🏀 ${escapeHtml(title)}</h2><div class="meta-row"><span>📅 ${escapeHtml(base.date||'')}</span><span>🏆 ${escapeHtml(base.event||'')}</span><span>📍 ${escapeHtml(base.place||'')}</span><span>${num(base.quarters||4)}Q</span></div></div><div class="score-block"><span>最終スコア</span><b>${scoreText(base)}</b><em>${escapeHtml(gameResult(base)||'')}</em></div></div>`; }}
  const sub = `対象試合 ${a.gameCount}試合`;
...
function chartRowsFor(t,games,playerId){ return games.slice(-8).map(g=>{ const a=aggregate({type:'game',key:g.id,playerId}); return {label:(g.date||'').slice(5), value:a.perGame.pts}; }); }
