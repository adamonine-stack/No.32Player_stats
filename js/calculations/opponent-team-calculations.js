...
export function placementLabelToRank(label){if(label==='優勝')return 'S';if(label==='準優勝')return 'A+';if(['3位','4位'].includes(label))return 'A';if(['5位','6位','7位','8位'].includes(label))return 'B+';if(label==='ベスト16')return 'B';if(label==='ベスト32')return 'C';return 'D'}
