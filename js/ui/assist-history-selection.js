(()=>{
  ...
  const eventIds=action=>{try{return JSON.parse(action?.dataset.historyEventIds||'[]')}catch{return []}};
  const scrollToAction=(container,id)=>{
    if(!container||!id)return false;
    const action=[...container.querySelectorAll('[data-history-action-id]')].find(el=>el.dataset.historyActionId===id||eventIds(el).includes(id));
    if(!action)return false;
    requestAnimationFrame(()=>action.scrollIntoView({block:'center',behavior:'auto'}));
    return true;
  };
  function rememberHistory(){
    ...
    if(returnActionId){scrollToAction(list,returnActionId);returnActionId='';}
  }
  function rememberSource(){
    ...
    sourceActionId='';
    const sourceButton=card.querySelector('[data-assist-source-event],[data-assist-event-id],[data-history-action-id]');
    sourceActionId=sourceButton?.dataset.assistSourceEvent||sourceButton?.dataset.assistEventId||sourceButton?.dataset.historyActionId||'';
    if(!sourceActionId&&historySnapshot){
      const doc=new DOMParser().parseFromString(`<div>${historySnapshot}</div>`,'text/html');
      const astActions=[...doc.querySelectorAll('[data-history-action-id]')].filter(el=>/AST/.test(el.textContent||''));
      const label=(sourceLabel.match(/(?:#|No\.?\s*)?\d+/i)||[])[0]||'';
      const match=[...astActions].reverse().find(el=>!label||(el.textContent||'').includes(label));
      sourceActionId=match?.dataset.historyActionId||eventIds(match)[0]||'';
    }
  }
  function convertCandidateModal(){
    ...
    card.querySelectorAll('[data-history-action-id]').forEach(action=>{
      ...
      if(id&&row){
        ...
        const choose=()=>{if(action.classList.contains('saving'))return;action.classList.add('saving');returnActionId=id;candidates.get(id).click()};
    ...
    card.querySelector('#assistHistorySelectBack').onclick=()=>{returnActionId=sourceActionId;back?.click()};
    const list=card.querySelector('.assist-history-selection-list');
    if(!scrollToAction(list,sourceActionId)){
