const CACHE_NAME = 'r32-shell-20260916-national-team-power-floor-v1';
const APP_SHELL = [
  './', './index.html', './manifest.json', './styles/quick-keyboard.css?v=20260906-v2', './js/ui/quick-keyboard.js?v=20260905-v1', './js/ui/quick-input-touch-fix.js?v=20260906-mouse-v2', './js/ui/quick-latest-history-controls.js?v=20260909-v1', './js/ui/quick-shot-tap-pc-fix.js?v=20260912-pc-flow-v2', './js/ui/game-history-fixed-nav.js?v=20260906-final-action-v1', './js/ui/game-history-edit-position.js?v=20260905-v1', './js/ui/pending-history-rehydrate.js?v=20260908-quarter-session-v2', './js/ui/game-list-registration-status.js?v=20260906-v1', './js/calculations/game-list-status-calculations.js',
...
self.addEventListener('activate', event => { event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith('r32-shell-') && key !== CACHE_NAME).map(key => caches.delete(key)))).then(() => self.clients.claim())); });
self.addEventListener('fetch', event => {
  ...
  if (requestUrl.origin === self.location.origin) event.respondWith(caches.match(event.request).then(cached=>cached||fetch(event.request).then(response=>{if(response.ok)caches.open(CACHE_NAME).then(cache=>cache.put(event.request,response.clone()));return response;})));
});
